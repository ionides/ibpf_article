...
if(LIK_ORDER){
  ...
  v5a <- read.csv(file="v5a_3/global.csv")
  v5b <- read.csv(file="v5b_3/global.csv")
  v5c <- read.csv(file="v5c_3/global.csv")
} else {
...
if(REMAKE){
  ...
  pdf(file="v5a-vs-v4a.pdf")
  x <- rep(v4a$loglik[1:18],each=2)
  y <- v5a$loglik
  x_lim <- range(c(x,truth_loglik,max(y)))
  y_lim <- range(c(y,truth_loglik,max(x)))
  plot(x,y,xlim=x_lim,ylim=y_lim,
  ...
  pdf(file="v5b-vs-v4b.pdf")
  x <- rep(v4b$loglik[1:18],each=2)
  y <- v5b$loglik
  x_lim <- range(c(x,truth_loglik,max(y)))
  y_lim <- range(c(y,truth_loglik,max(x)))
  plot(x,y,xlim=x_lim,ylim=y_lim,
