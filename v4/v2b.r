# v4 is the same as v3 except restarting the top 25% not top 50%
# v3 is a new attempt at maximizing simulated data from the he10 model

# the goal is to work out what it takes to make reasonable progress on the
# last 100 units or so of log likelihood, which can be problematic when
# there are many parameters.

devel_version <- "v2"
old_version <- "v1" # used to read in starting values when continuation=TRUE
run_version <- "b"
seed_offset <- 1
...
sim_data <- TRUE # fit simulated data rather than the actual data
...
print(as.data.frame(results[,c("loglik","loglik.se",estParNames_expanded[1:5])]))
