...
run_version <- "c"
code_version <- paste0("v1",run_version)
run_level <- 3

estimated <- switch(run_version,
  a="mostly shared",
  b="all unit-specific",
  c="he10") # he10 has all unit-specific with immigration but not coupling
...
# use the full data size for each run level, to help with debugging.
U <- 20
N <- 730
...
saveRDS(file=paste0(out_dir,"model.rds"),sim)
...
#reg_rw.sd <- lapply(regEstParNames_expanded, function(x) rw_r)
reg_rw.sd <- rep(list(rw_r),times=length(regEstParNames_expanded))
#lapply(regEstParNames_expanded, function(x) rw_r)
...
if(!continuation){
  ...
  estParTransCenter <- coef(sim,estParNames_expanded,transform=TRUE)
  ...
  start_params <- old_results[,1:(length(coef(sim)))]
