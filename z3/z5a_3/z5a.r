# z3 is an attempt at maximizing likelihood for data & model of he10
# z3 modifies z2 by restarting the top 25% of searches rather than 50%

devel_version <- "z5"
old_version <- "z4" # used to read in starting values when continuation=TRUE
run_version <- "a"
...
# do_bake = FALSE manually avoids unintentional recomputation, e.g.,
# when editing on a different machine.
# do_bake <- FALSE 
do_bake <- TRUE

continuation <- TRUE # set to be false when starting a new search
if(continuation){
...
library(doRNG)
registerDoRNG(4454+seed_offset)
...
set.seed(0)
results_file <- paste0(out_dir,"global.rds")
results <- if(do_bake){bake(file=results_file,
{
  registerDoRNG(1270401374+seed_offset)
...
)} else readRDS(results_file)

initial_lik_file <- paste0(out_dir,"initial_lik.rds")
if(compute_start_likelihood){
  initial_lik <- if(do_bake){
    bake(file=initial_lik_file,{
      registerDoRNG(3889100+seed_offset)
        foreach(guess=iter(guesses,by="row"), .combine=rbind) %dopar% {
        library(pomp)
        library(tidyverse)
      ...
      attr(initial_lik_eval,"ncpu") <- getDoParWorkers()
      initial_lik_eval
    })
  } else {
    readRDS(initial_lik_file)
  }
  results <- cbind(results,initial_lik)
  print(initial_lik)
} 
