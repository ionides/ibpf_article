...
The pseudocode in Alg.~\ref{alg:ibpfilter} represents our implementation of {\ibpf} as the R function \code{ibpf} extending the open-source package \code{spatPomp} \citep{asfaw21github,asfaw21arxiv}.
...
<<search_diagnostics, echo=F, fig.height=6.5, fig.width=4.5, out.width="5in", fig.cap = "Three steps of a likelihood search on simulated data (left panels) and UK measles data (right panels). Final log-likelihood values are plotted against starting log-likelihood for an initial search (top row), a refinement of this search (second row) and a second refinement (third row). Unit-specific parameters were $\\rho$, $S_0$, $E_0$, $I_0$ (for the simulation study, model A) and all parameters (for the data, model B).">>=
par(mfcol=c(3,2))
...
The population dynamics are written in terms of counting processes $N_{AB,\unit}(t)$ enumerating cumulative transitions in town $\unit$, up to time $t$, between compartments $A$ and $B$.
...
In \eqref{eq:dEdt}, $\alpha$ is a mixing exponent modeling inhomogeneous contact rates within a town, and $\iota$ models immigration of infected individuals which is appropriate when analyzing a subset of cities that cannot be treated as a closed system.
...
\]
where $\dist(\unit,\altUnit)$ denotes the distance between town $\unit$ and town $\altUnit$, $P_\unit$ is the average population for town $\unit$ across time, $\bar{P}$ is the average population across towns, and $\overline{\dist}$ is the average distance between a randomly chosen pair of towns.
Here, we model $v_{\unit\altUnit}$ as fixed through time and symmetric between any two arbitrary towns, though a natural extension would allow for temporal variation and asymmetric movement between two towns.
...
\noindent This work was supported by National Science Foundation grants DMS-1761603 and DMS 1646108.
